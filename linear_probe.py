...
def main(rank, logger_name, log_path, log_file):
    ...
    # -----  we should load pretrained weights before wrapping `model` with DDP
    # ref: https://pytorch.org/docs/stable/generated/torch.nn.parallel.DistributedDataParallel.html
    ...
    model.load_state_dict(pretrained, strict=False)    # it is necessary to set `strict`=False

    # ------ linear probe: freeze all matched weights and bias, which are in PerceiverEncoder
